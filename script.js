...
async function writeLineToDom(item) {
  ...
  newButton.addEventListener("mouseover", function () {
    newButton.classList.add("todo__label--delete");
  });
  newButton.addEventListener("mouseout", function () {
    newButton.classList.remove("todo__label--delete");
  });
}
...
btnAdd.addEventListener("click", async function () {
  ...
  const newTaskItem = document.getElementById("todo__new").value;
  if (newTaskItem == "") {
    ...
    var data = { todo: "", done: false, _id: "0" };
    data["todo"] = newName;
    writeLineToDom(data);
    document.getElementById("todo__new").value = "";
  }
});
